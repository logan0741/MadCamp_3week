...
try:
    ...
    if spec and spec.loader:
        size_scraper = importlib.util.module_from_spec(spec)
        sys.modules[spec.name] = size_scraper
        spec.loader.exec_module(size_scraper)
        scrape_musinsa_sizes = size_scraper.scrape_musinsa_sizes
    else:  # pragma: no cover
        scrape_musinsa_sizes = None
except Exception as exc:  # pragma: no cover
    scrape_musinsa_sizes = None
    logger.warning(f"backend size_scraper unavailable: {exc}")
